...
Puppet::Type.type(:chocolateyfeature).provide(:windows) do
  ...
  def properties
    if @property_hash.empty?
      # put nil here and it might error
      # # or do empty hash here so it fails
      #  @property_hash = query || { :ensure => ( :disabled )}
      #  @property_hash[:ensure] = :disabled if @property_hash.empty?
