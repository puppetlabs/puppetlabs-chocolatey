...
describe 'Chocolatey Common' do
  ...
  context ".chocolatey_command" do
    it "should find chocolatey install location based on PuppetX::Chocolatey::ChocolateyInstall", :if => Puppet::Util::Platform.windows? do
    ...
    it "should find chocolatey install location based on default location", :if => Puppet::Util::Platform.windows? do
      PuppetX::Chocolatey::ChocolateyInstall.expects(:install_path).returns('c:\dude')
